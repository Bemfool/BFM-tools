...
int load(d_type type) {
	...
	if (type == BINARY_DATA) {
		...
		load_text_model(shape_mu, TEXT_DATA);
		load_text_model(shape_ev, TEXT_DATA);
		load_pc_model(shape_pc, TEXT_DATA);
		load_text_model(tex_mu, TEXT_DATA);
		load_text_model(tex_ev, TEXT_DATA);
		load_pc_model(tex_pc, TEXT_DATA);
		load_text_model(tl, TEXT_DATA);
	}
